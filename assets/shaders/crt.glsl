...
vec4 crt(vec4 color) {
    ...
    vec2 uvG = vec2(0, 0);
    vec2 uvB = vec2(0, 0);
    vec4 col = get(v_coord, vec4(1, 0, 0, 1), -u_chromatic_aberration, uvR) + get(v_coord, vec4(0, 1, 0, 1), 0, uvG) + get(v_coord, vec4(0, 0, 1, 1), u_chromatic_aberration, uvB);
